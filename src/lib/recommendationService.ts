...
/**
 * AI-based explanation generation (your original logic)
 */
async function generateAIExplanation(
  ...
  const prompt = `Generate a ~600-character explanation for why ${currentUser.username} and ${userA.username} should connect. The first 300 characters should be a narrative of ${userA.username}, and the second 300 characters should be on the similarities between ${currentUser.username} [${userASummary}] and ${userA.username} [${userBSummary}]`;
  
  const response = await openai.chat.completions.create({
    model: "gpt-4-turbo", // or your preferred model
    messages: [
      { role: "system", content: "You are an assistant that helps explain connections between users of a social platform based on their shared interests and thoughts." },
